 ...
 * @api private
 */
MqttClient.prototype._setupStream = async function () {
  var connectPacket
  var that = this
  ...
  console.log('[MQTT] _setupStream :: using streamBuilder provided to client to create stream')
  this.stream = await this.streamBuilder(this)
...
MqttClient.prototype._reconnect = function () {
  ...
  this.emit('reconnect')
  if (this.connected) {
    this.end( async () => { await this._setupStream() })
...
MqttClient.prototype._setupReconnect = function () {
  ...
  if (!that.disconnecting && !that.reconnectTimer && (that.options.reconnectPeriod > 0)) {
    ...
    that.reconnectTimer = setInterval(function () {
      console.log('_setupReconnect : checking connected status :' + this.connected)
      if(!this.connected) {
...
MqttClient.prototype._flushStoreProcessingQueue = function () {
  for (var f of this._storeProcessingQueue) {
